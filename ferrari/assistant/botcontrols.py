...
@feri.bot_cmd(pattern="^اذع$", from_users=Config.OWNER_ID)
async def bot_broadcast(event):
    replied = await event.get_reply_message()
    if not replied:
        return await event.reply("**- يجب عليم الرد على رسالة اولا لعمل اذاعة**")
...
@feri.ar_cmd(pattern="^المستخدمين$")
...
@feri.bot_cmd(pattern="^حظر\\s+([\\s\\S]*)", from_users=Config.OWNER_ID)
...
@feri.bot_cmd(pattern="^الغاء حظر(?:\\s|$)([\\s\\S]*)", from_users=Config.OWNER_ID)
async def ban_botpms(event):
    user_id, reason = await get_user_and_reason(event)
    reply_to = await reply_id(event)
    if not user_id:
...
@feri.ar_cmd(pattern="التكرار (تفعيل|تعطيل)$")
