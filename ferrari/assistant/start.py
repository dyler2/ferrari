...
from ferrari import Config
from ferrari import ferrari as feri
...
@feri.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@feri.bot_cmd(pattern="^ايدي$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await feri.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await feri.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await feri.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await feri.tgbot.get_entity(BOTLOG_CHATID)
            await feri.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await feri.tgbot.send_message(BOTLOG_CHATID, "**الغاء ححظر بوتك !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@feri.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await feri.get_entity(user_id)
    except Exception as e:
...
@feri.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@feri.bot_cmd(incoming=True, func=lambda e: e.is_private)
@feri.bot_cmd(edited=True, func=lambda e: e.is_private)
