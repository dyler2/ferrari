...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧⵧⵧferrariⵧⵧⵧⵧⵧ⍣**\n[𝐹𝐸𝑅𝑅𝐴𝑅𝐼 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 🧸♥](https://t.me/ferrarisrc)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**ferrari"):
            ...
            builder = event.builder
            troll = os.path.join("./ferrari", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./ferrari", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./ferrari", "hide.txt")
            try:
            ...
            result = builder.article(
                title="ferrari Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب فيراري **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/ferrarisrc"),
                Button.url(
                    "المطور",
                    "t.me/jj8jjj8",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل فيراري من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝐹𝐸𝑅𝑅𝐴𝑅𝐼 𝙐𝙎𝙀𝙍𝘽𝙊𝙏 ](https://t.me/ferrarisrc)",
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
