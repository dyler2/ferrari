...
import ferrari
from ferrari import BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
try:
    ...
    ferrari.loop.run_until_complete(setup_bot())
...
try:
    ...
    ferrari.loop.create_task(saves())
...
try:
    ...
    ferrari.loop.run_until_complete(mybot())
...
async def startup_process():
    if ferrari.uid in blacklisted_users:
        LOGS.info("انت لا يمكنك تنصيب سورس فيراري عزيزي دي")
        return
    if not gvarstatus("TNSEEB"):
        try:
            ...
            LOGS.info(
                ...
                \nمجموعة قناة السورس  https://t.me/FERRARI_support"
            )
            LOGS.info("============================================================")
            await verifyLoggerGroup()
...
ferrari.loop.run_until_complete(startup_process())

if len(sys.argv) in {1, 3, 4}:
    with contextlib.suppress(ConnectionError):
        ferrari.run_until_disconnected()
else:
    ferrari.disconnect()
