...
from . import ferrari, song_download
...
@ferrari.ar_cmd(pattern="بحث(320)?(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="فيديو(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="(ا(ل)?ا(س)?م)(?:\s|$)([\s\S]*)")
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await ferrari(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
