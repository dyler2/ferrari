...
from . import CMD_HELP, CMD_LIST, SUDO_LIST, edit_delete, edit_or_reply, ferrari
...
def plug_checker(plugin):
    plug_path = f"./ferrari/plugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./xtraplugins/{plugin}.py"
    return plug_path


@ferrari.ar_cmd(pattern="تنصيب$")
...
@ferrari.ar_cmd(pattern="الغاء تنصيب ([\s\S]*)")
