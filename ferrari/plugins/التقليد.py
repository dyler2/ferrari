...
@ferrari.ar_cmd(pattern="تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "⌔∮ يرجى الرد على الشخص الذي تريد ازعاجه ،")
    ferrarievent = await edit_or_reply(event, "⌔∮ يتم تفعيل هذا الامر انتظر قليلا ،")
    user, rank = await get_user_from_event(event, ferrarievent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(ferrarievent, f"⌔∮ خطأ\n`{str(e)}`")
    else:
        await edit_or_reply(
            ferrarievent,
...
@ferrari.ar_cmd(pattern="الغاء تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@ferrari.ar_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@ferrari.ar_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@ferrari.ar_cmd(incoming=True, edited=False)
