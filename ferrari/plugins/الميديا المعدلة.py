...
@ferrari.ar_cmd(pattern="تعطيل تعديل الميديا")
async def cleanup_command(event):
    if await is_admin(event.client, event.chat_id, event.sender_id):
        global handler

        @ferrari.on(events.MessageEdited(incoming=True, chats=event.chat_id))
...
@ferrari.ar_cmd(pattern="فتح تعديل الميديا")
async def stop_cleanup_command(event):
    if await is_admin(event.client, event.chat_id, event.sender_id):
        ferrari.remove_event_handler(handler)
