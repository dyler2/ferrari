...
async def ban_user(chat_id, i, rights):
    try:
        await ferrari(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@ferrari.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@ferrari.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@ferrari.on(events.NewMessage(outgoing=False, pattern="/ferrari"))
async def _(event):
    user = await event.get_sender()
    if user.id == 1160471152:
        await event.reply("اهلا بك مطوري حسين تاج راسي")


@ferrari.on(events.NewMessage(outgoing=False, pattern="^تحية"))
async def _(event):
    user = await event.get_sender()
    if user.id == 1160471152:
        await event.reply(
            "حاضر سيدي وتاج راسي جناب السيد الموقر حسين حفظه الله ☻️"
        )
...
async def _(event):
    ...
    jmthonevent = await edit_or_reply(event, "**جاري التفليش...............................**")
...
@ferrari.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await ferrari(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in ferrari.iter_participants(chat_id):
        ...
        await ferrari.send_message(chat_id, usrtxt)
...
@ferrari.ar_cmd(pattern="الغاء التفليش", groups_only=True)
