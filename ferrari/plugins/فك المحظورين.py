...
async def unblock_users(ferrari):
    @ferrari.on(events.NewMessage(outgoing=True, pattern=".فك المحظورين"))
    async def _(event):
        list = await ferrari(GetBlockedRequest(offset=0, limit=1000000))
        if len(list.blocked) == 0:
            razan = await event.edit(f"- لم تقم بحظر اي شخص اصلا .")
        else:
            unblocked_count = 1
            for user in list.blocked:
                UnBlock = await ferrari(UnblockRequest(id=int(user.peer_id.user_id)))
                unblocked_count += 1
...
loop.create_task(unblock_users(ferrari))
