...
async def delpack(jmthonevent, conv, args, packname):
    try:
        ...
        await ferrari(unblock("stickers"))
        await conv.send_message("/delpack")
...
@ferrari.ar_cmd(pattern="ملصق(?:\s|$)([\s\S]*)")
async def kang(args):
    photo = None
    emojibypass = False
    is_anim = False
...
@ferrari.ar_cmd(pattern="حزمة(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="تحويل بملصق(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="معلومات_ألملصق$")
