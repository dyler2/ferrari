...
from . import BOTLOG, BOTLOG_CHATID, ferrari
...
@ferrari.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    ferrarievent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if not event.reply_to_msg_id:
        return await ferrarievent.edit(
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await ferrarievent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await ferrarievent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
        ...
        ms = (end - start).seconds
        ferrari = f"https://graph.org/{response['path']}"
        await ferrarievent.edit(
            f"**الرابط**:  [اضغط هنا]({ferrari})\
