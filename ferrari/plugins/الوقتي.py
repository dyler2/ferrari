...
from . import edit_delete, ferrari, logging
...
autopic_path = os.path.join(os.getcwd(), "ferrari", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "ferrari", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "ferrari", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await ferrari.upload_file(autophoto_path)
        try:
            if i > 0:
                await ferrari(
                    functions.photos.DeletePhotosRequest(
                        await ferrari.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await ferrari(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await ferrari(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await ferrari(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@ferrari.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@ferrari.ar_cmd(pattern="اسم وقتي$")
async def _(event):
...
@ferrari.ar_cmd(pattern="بايو وقتي$")
async def _(event):
...
@ferrari.ar_cmd(pattern="انهاء ([\s\S]*)")
async def _(event):
...
ferrari.loop.create_task(digitalpicloop())
ferrari.loop.create_task(autoname_loop())
ferrari.loop.create_task(autobio_loop())
