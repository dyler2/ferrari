...
@ferrari.ar_cmd(pattern="كروباتي$")
async def oeo(event):
    result = await ferrari(functions.channels.GetGroupsForDiscussionRequest())
    alist = []
    for item in result.chats:
        ...
        alist.append(roz)
    if alist:
        await ferrari.send_message("me", "\n".join(alist))


@ferrari.ar_cmd(pattern="الحاظرهم$")
async def main(event):
    result = await ferrari(
...
@ferrari.ar_cmd(pattern="نوعه (.*)")
async def se(event):
    exe = event.text[5:]
    x = await ferrari.get_entity(exe)
...
@ferrari.ar_cmd(pattern="احذف (.*)")
async def se(event):
    exe = event.text[5:]
    await ferrari.get_dialogs()
    chat = exe
    await ferrari.delete_dialog(chat, revoke=True)
