...
@ferrari.ar_cmd(pattern="تفعيل الترجمة")
...
@ferrari.ar_cmd(pattern="تعطيل الترجمة")
...
@ferrari.on(events.NewMessage(outgoing=True))
    ...
    if gvarstatus("translateen"):
        ...
        try:
            ...
            if translation.text != event.message.message:
                await ferrari.edit_message(event.message, translation.text)
        except:
            pass
