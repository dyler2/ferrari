...
@ferrari.ar_cmd(pattern="اوامر التحكم")
async def hi(event):
    await edit_or_reply(
        event,
        "**[𝐹𝐸𝑅𝑅𝐴𝑅𝐼 - 𝐂𝐌𝐃.](t.me/ferrarisrc)**/n✦┅━╍━╍╍━━╍━━╍━┅✦\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب فيراري حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر  وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب فيراري الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين",
        link_preview=False,
    )
...
@ferrari.ar_cmd(pattern="التحكم (تفعيل|تعطيل)$")
...
@ferrari.ar_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@ferrari.ar_cmd(pattern="المتحكمين$")
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "**• يبدو انك لم تقوم بأضافة مستخدم متحكم لفيراري الخاص بك**"
        )
    result = "**قائمة المستخدمين المتحكمين بتنصيب فيراري :**\n\n"
...
ferrari.loop.create_task(_init())
