...
@ferrari.ar_cmd(pattern="الحاسبة(?:\s|$)([\s\S]*)")
async def icalc(e):
    if e.client._bot:
        return await e.reply("**الحاسبة العلمية لسورس فيراري\n @ferrari**", buttons=lst)
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحاسبة العلمية لسورس فيراري\n @ferrari**", buttons=lst
        )
        await event.answer([calc])


# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @ferrari  ~ @RR7PP
@ferrari.tgbot.on(CallbackQuery(data=re.compile(b"calc(.*)")))
@check_owner
...
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @ferrari  ~ @RR7PP
@ferrari.tgbot.on(CallbackQuery(data=re.compile(b"recalc")))
@check_owner
async def _(e):
    m = [
        "AC",
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحاسبة العلمية لسورس فيراري\n @ferrari**", buttons=lst)
