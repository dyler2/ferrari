...
@ferrari.ar_cmd(pattern="وقت(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await edit_delete(
            event,
...
@ferrari.ar_cmd(pattern="استخدامي$")
...
@ferrari.ar_cmd(pattern="زخرفة الوقت (.*)")
async def variable(var):
    if Config.HEROKU_API_KEY is None:
        return await edit_delete(
            var,
