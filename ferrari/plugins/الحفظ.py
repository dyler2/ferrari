...
@ferrari.ar_cmd(pattern="حفظ كتابة$")
async def save(e):
...
@ferrari.ar_cmd(pattern="حفظ توجيه$")
async def saf(e):
    razan = await e.get_reply_message()
    if not razan:
        return await edit_delete(
...
@ferrari.ar_cmd(pattern="همسة ?(.*)")
async def roz(event):
    razan = event.pattern_match.group(1)
    BE = "@whisperBot"
    if event.reply_to_msg_id:
        await event.get_reply_message()
    R7 = await ferrari.inline_query(BE, razan)
    await R7[0].click(event.chat_id)
    await event.delete()


@ferrari.ar_cmd(pattern="ايجاد الفايروسات$")
async def _(event):
