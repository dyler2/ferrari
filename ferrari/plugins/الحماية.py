...
from ferrari.core.logger import logging
...
# ترجمه وكتابة فريق جمثون
@ferrari.ar_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@ferrari.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
# ترجمه وكتابة فريق جمثون
@ferrari.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
# ترجمه وكتابة فريق جمثون
@ferrari.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
# ترجمه وكتابة فريق جمثون
@ferrari.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
@ferrari.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق جمثون
...
@ferrari.ar_cmd(pattern="(س|سماح)(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@ferrari.ar_cmd(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
@ferrari.ar_cmd(pattern="انبلوك(?:\s|$)([\s\S]*)")
...
# ترجمه وكتابة فريق جمثون
@ferrari.ar_cmd(pattern="المسموح لهم$")
async def approve_p_m(event):
    if gvarstatus("pmpermit") is None:
        return await edit_delete(
            event,
