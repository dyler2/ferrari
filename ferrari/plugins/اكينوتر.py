...
@ferrari.ar_cmd(pattern="اكينوتر(?:\s|$)([\s\S]*)")
async def rozdo(e):
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(b"aki_?(.*)")))
@check_owner
async def daj(e):
...
@ferrari.tgbot.on(CallbackQuery(data=re.compile(b"aka_?(.*)")))
@check_owner
async def rooks(e):
    ...
    if (
        ...
        ans = [
            await e.builder.photo(
                aki_photo,
                text=query,
                buttons=[Button.inline(" ابدأ اللعب", data=e.text)],
            )
        ]
        await e.answer(ans)
