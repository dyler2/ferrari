...
from ferrari import Convert, ferrari
...
@ferrari.ar_cmd(pattern="دائري ?((-)?s)?$")
...
@ferrari.ar_cmd(pattern="(لصورة|تحويل صورة)$")
async def _(event):
...
@ferrari.ar_cmd(pattern="(لملصق|تحويل ملصق)$")
async def _(event):
    reply_to_id = await reply_id(event)
    reply = await event.get_reply_message()
    if not reply:
...
@ferrari.ar_cmd(pattern="لملف ([\s\S]*)")
async def get(event):
    name = event.text[5:]
    if name is None:
...
@ferrari.ar_cmd(pattern="الملف لصورة$")
...
@ferrari.ar_cmd(pattern="(تحويل متحركة|لمتحركة)$")
async def _(event):
...
@ferrari.ar_cmd(pattern="تحويل الى (صوتي|بصمة)")
async def _(event):
