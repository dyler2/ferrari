...
async def participant_check(channel, user_id):
    try:
        await ferrari(GetParticipantRequest(channel, int(user_id)))
        return True
...
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, ferrari.uid):
            return await event.reply(
...
@ferrari.ar_cmd(pattern="تعطيل الاجباري")
