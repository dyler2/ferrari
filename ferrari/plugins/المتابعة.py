...
from ferrari.sql_helper.MOrakb import *

from ..Config import Config

# from ferrari.sql_helper.mo5zn import *


@ferrari.ar_cmd(incoming=True, func=lambda e: e.is_group, edited=False, forword=None)
...
@ferrari.ar_cmd(pattern=r"تخزين")
async def add_morakb_handler(event):
    msg = event.text.split()
    username = msg[1]
    user = await ferrari.get_entity(username)
...
@ferrari.ar_cmd(pattern=r"الغاء تخزين")
...
@ferrari.ar_cmd(pattern=r"متابعة")
async def add_morakb_handler(event):
    msg = event.text.split()
    username = msg[1]
    user = await ferrari.get_entity(username)
...
@ferrari.ar_cmd(pattern=r"الغاء متابعة")
async def remove_morakb_handler(event):
    msg = event.text.split()
    username = msg[2]
    user = await ferrari.get_entity(username)
