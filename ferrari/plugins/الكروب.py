...
@ferrari.ar_cmd(pattern="المشرفين(?: |$)(.*)")
async def _(event):
...
@ferrari.ar_cmd(pattern="البوتات(?: |$)(.*)")
async def _(event):
...
@ferrari.ar_cmd(pattern="الاعضاء(?: |$)(.*)")
async def get_users(show):
    ...
    ferrarievent = await edit_or_reply(show, "**⌔∮ جاري سحب قائمة معرّفات الأعضاء 🝛**")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(ferrarievent, mentions)


@ferrari.ar_cmd(pattern="معلومات(?: |$)(.*)")
async def info(event):
    ferrarievent = await edit_or_reply(event, "**⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏**")
    chat = await get_chatinfo(event, ferrarievent)
    caption = await fetch_info(chat, event)
    try:
        await ferrarievent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**⌔∮ هناك خطأ في معلومات الدردشة ✕ : **\n`{str(e)}`"
            )
        await ferrarievent.edit("**⌔∮ حدث خطأ ما، يرجى التحقق من الأمر ⎌**")


async def get_chatinfo(event, ferrarievent):
    ...
    try:
        ...
        try:
            ...
            await ferrarievent.edit("**⌔∮ لم يتمّ العثور على القناة/المجموعة ✕**")
            return None
        except ChannelPrivateError:
            await ferrarievent.edit(
                "**⌔∮ هذه مجموعة أو قناة خاصة أو لقد تمّ حظري منه ⛞**"
            )
            return None
        except ChannelPublicGroupNaError:
            await ferrarievent.edit("**⌔∮ القناة أو المجموعة الخارقة غير موجودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            await ferrarievent.edit(str(err))
            return None
    return chat_info
